...
var utils = {
    ...
    path: {
        ...
        /**
         * Get absolute url from a url that bases on current context.
         */
        getAbsoluteUrl: function (url, rootUrl, baseUrl) {
            if (utils.path.isAbsolute(url))
                return url;

            if (!rootUrl) rootUrl = app.baseUrl;
            if (!baseUrl) baseUrl = app.baseUrl;

            if (url === '~') return rootUrl;
            else if (url === '.') return baseUrl;
            else if (url.startsWith('~/'))
                return utils.path.combine(rootUrl, url.substring(2));
            else if (url.startsWith('./'))
                return utils.path.combine(baseUrl, url.substring(2));
            else
                return utils.path.combine(baseUrl, url);
        },
        ...
        combine: function (path1, path2) {
            ...
            if (!utils.isNullOrEmpty(vp1))
                paths.push(vp1);
            if (!utils.isNullOrEmpty(vp2))
                paths.push(vp2);
...
var bus = (function () {
    ...
    return {
        ...
        /**
         * Raises event. Just like raise() excepts args is a list of argument
         */
        emit: function (name, args) {
            if (!args) args = [];
            args.unshift(name);
            return this.raise.apply(this, args);
        },
...
Runtime.prototype = {
    ...
    ready: function (cb) {
        if (!this.moduleInstance) {
            ...
            if (!this.isReference) {
                ...
                steal.import(url).then(function(fn){
    ...
    handleCallbacks: function(){
        while(this.callbacks.length){
            var cb = this.callbacks.splice(0, 1)[0];
            if(cb) cb(this.moduleInstance);
        }
        this.isLoading = false;
    }
};
...
Sandbox.prototype = {
    ...
    getAbsoluteUrl: function (url) {
        return utils.path.getAbsoluteUrl(url, app.baseUrl, this.context.baseUrl);
    },
    /**
     * Imports a module from the specified paths.
     */
    import: function (urls, callback) {
        if(typeof(urls) === 'string') urls = [urls];
        var _urls = [];
        utils.each(urls, function(url){
            _urls.push(this.getAbsoluteUrl(url));
        }, this);
        
        steal.import.apply(steal, _urls).then(function(){
...
var app = {
    ...
    start: function (options) {
        var addinItems = options.addins || [];
        var addinsFolder = options.addinUrl || "addins";
        ...
        addinItems = utils.filter(addinItems, function (config) {
            return config.disabled !== true;
        });

        utils.each(addinItems, function (addinItem) {
            var path = utils.path.getAbsoluteUrl(addinItem.url, app.baseUrl, addinsFolder);
            var baseUrl = utils.path.getParent(path);

            addinItem.baseUrl = baseUrl;
            addinItem.url = path;
        });

        var loadAddInFileFnList = addinItems.map(function (config) {
        ...
        utils.async.parallel(loadAddInFileFnList, function (err, results) {
            if (!err) {
                utils.each(results, function (data, idx) {
                    var xml = (data instanceof String) ? utils.dom.parseXML(data) : data;
                    var reader = new XmlObject(xml);
                    if (reader.moveTo("AddIn")) {
                        var props = Properties.read(reader);
                        var baseUrl = addinItems[idx].baseUrl;
                        props.set('baseUrl', baseUrl);
                        var addin = new AddIn(new Sandbox({
                            rootUrl: app.baseUrl,
                            baseUrl: baseUrl
                        }), props);
            ...
            else logger.error('Error during loading addin files: ', err);
        });
    }
};
