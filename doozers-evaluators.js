        ...
        var classDoozer = {
            ...
            loadRuntime: function (args, cb) {
                ...
                if (!utils.isNullOrEmpty(path)) {
                    var require = sandbox.services.get('requirejs');
                    var baseUrl = codon.addIn.props.get('baseUrl');
                    var url = utils.path.combine(baseUrl, path);
                    require(url, function (fn) {
                        var instance = fn(sandbox);
                        cb(instance);
                    });
                } else {
                    var runtime = sandbox.runtimes.get(from, codon.addIn);
                    if (!runtime)
